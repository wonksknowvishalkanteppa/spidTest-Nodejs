            ...
            if (err) {
                ...
                if (result.type === "result") {
                    ...
                    let response = {
                        ...
                        "packet loss": result.packetLoss === undefined ?
                            "0.00%" : result.packetLoss.toFixed(2) + "%",
                        isp: result.isp,
    ...
    exec("iperf3 -J -c " + host + " -p " + port, (err, stdout, stderr) => {
        if (err) {
            ...
            if (result.error) {
                ...
                let sender = {
                    interval: result.sum_sent.start.toFixed(2) +
                        "-" +
                ...
                let receiver = {
                    interval: result.sum_received.start.toFixed(2) +
                        "-" +
                        result.sum_sent.end.toFixed(2) +
                        " sec",
